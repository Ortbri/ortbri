...
export default function PhysicsScene({ className }: PhysicsSceneProps) {
	...
	useEffect(() => {
		...
		const leftWallBody = new CANNON.Body({
			...
			shape: new CANNON.Box(
				new CANNON.Vec3(wallThickness, wallHeight, width / 20),
			),
		});
		...
		const backWallBody = new CANNON.Body({
			type: CANNON.Body.STATIC,
			shape: new CANNON.Box(
				new CANNON.Vec3(width / 20, wallHeight, wallThickness),
			),
		});
	...
	return (
		<>
			...
			<div className="fixed bottom-4 left-4 z-10 flex flex-col gap-2 p-4 bg-black/30 rounded-lg backdrop-blur-sm">
				<h2 className="text-white font-bold mb-2">Controls</h2>
				...
				<div className="grid grid-cols-2 gap-2 mb-4">
					...
					<button
						type="button"
						onClick={() => addCube(50)}
						className="rounded-md bg-purple-500 px-4 py-2 text-white hover:bg-purple-600 transition-colors"
					>
						Add 50 Cubes
					</button>
				...
				<div className="mb-2">
					<label htmlFor="gravity" className="block text-white mb-1">
				...
				<div className="rounded-md bg-black/50 px-4 py-2 text-white">
					Cubes: {cubeCount}
				</div>
			</div>
		</>
